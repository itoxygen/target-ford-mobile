...
public class LoginManager {
	...
	private void showLoginUIInternal(final String startUrl, final String endUrl, final Context context, LoginUIOperationCallback callback) {
        ...
        if (Build.VERSION.SDK_INT < 18) { // A fix
            wv.getSettings().setSavePassword(false);
            wv.getSettings().setAppCacheEnabled(false);
            wv.getSettings().setDatabaseEnabled(false);
		...
		wv.setWebViewClient(new WebViewClient() {
			...
			public void onPageStarted(WebView view, String url, Bitmap favicon) {
                Log.d("LoginManager", "onPageStarted() url=" + url);
			...
			private boolean isFinalUrl(String url) {
				...
				return url.startsWith(endUrl) && !url.startsWith(startUrl); // Another fix
			}
			...
			public void onPageFinished(WebView view, String url) {
                Log.d("LoginManager", "onPageFinished() url=" + url);
